...
(function($) {
	var scrollMethods={
		init:function(options){
			...
			this.each(function(){
				if(o.autoAdd>0){
					o.curNumber=scrollMethods._autoAdd.call(self,o.curNumber, o.autoAdd);
				}
				//获得之前的数据
		...
		_autoAdd:function(curNumber,bits){
			var tempLen=curNumber.toString().length;
			var noeNumber=curNumber;
			if(tempLen<bits){
				
				for(var i=0;i<(bits-tempLen);i++){
					noeNumber="0"+noeNumber;
				}
				return noeNumber;
			}else{
				return curNumber
